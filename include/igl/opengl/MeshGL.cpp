...
R"(#version 150
  ...
  void main()
  {
    ...
    vec3 light_position_eye_2 = -light_position_eye; // Added by Terry
    vec3 light_position_eye_3 = vec3(0.0, light_position_eye.y, light_position_eye.x); // Added by Terry
    vec3 light_position_eye_4 = vec3(0.0, light_position_eye.y, -light_position_eye.x); // Added by Terry

    vec3 vector_to_light_eye = light_position_eye - position_eye;
    vec3 vector_to_light_eye_2 = light_position_eye_2 - position_eye; // Added by Terry
    vec3 vector_to_light_eye_3 = light_position_eye_3 - position_eye; // Added by Terry
    vec3 vector_to_light_eye_4 = light_position_eye_4 - position_eye; // Added by Terry
    vec3 direction_to_light_eye = normalize (vector_to_light_eye);
    vec3 direction_to_light_eye_2 = normalize (vector_to_light_eye_2); // Added by Terry
    vec3 direction_to_light_eye_3 = normalize (vector_to_light_eye_3); // Added by Terry
    vec3 direction_to_light_eye_4 = normalize (vector_to_light_eye_4); // Added by Terry
    float dot_prod = dot (direction_to_light_eye, normalize(normal_eye));
    float dot_prod_2 = dot(direction_to_light_eye_2, normalize(normal_eye)); // Added by Terry
    float dot_prod_3 = dot(direction_to_light_eye_3, normalize(normal_eye)); // Added by Terry
    float dot_prod_4 = dot(direction_to_light_eye_4, normalize(normal_eye)); // Added by Terry
    float clamped_dot_prod = max (dot_prod, 0.0);
    clamped_dot_prod = max(clamped_dot_prod, dot_prod_2); // Added by Terry
    clamped_dot_prod = max(clamped_dot_prod, dot_prod_3); // Added by Terry
    clamped_dot_prod = max(clamped_dot_prod, dot_prod_4); // Added by Terry
    vec3 Id = Ld * vec3(Kdi) * clamped_dot_prod;    // Diffuse intensity

    vec3 reflection_eye = reflect (-direction_to_light_eye, normalize(normal_eye));    
    //vec3 reflection_eye_2 = reflect(-direction_to_light_eye_2, normalize(normal_eye)); // Added by Terry
    //vec3 reflection_eye_3 = reflect(-direction_to_light_eye_3, normalize(normal_eye)); // Added by Terry
    //vec3 reflection_eye_4 = reflect(-direction_to_light_eye_4, normalize(normal_eye)); // Added by Terry
    //reflection_eye = max(reflection_eye, reflection_eye_2); // Added by Terry
    //reflection_eye = max(reflection_eye, reflection_eye_3); // Added by Terry
    //reflection_eye = max(reflection_eye, reflection_eye_4); // Added by Terry
