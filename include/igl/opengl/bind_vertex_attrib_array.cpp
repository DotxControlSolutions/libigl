    ...
    const GLuint n_rows)
{
    ...
    glBindBuffer(GL_ARRAY_BUFFER, bufferID);
    if (refresh) {
        glBufferData(GL_ARRAY_BUFFER, sizeof(float) * M.size() * n_rows, nullptr, GL_DYNAMIC_DRAW);
        glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(float) * M.size(), M.data());
    }
    ...
    GLuint step)
{
    ...
    if (step >= 5)
        if (refresh) {
            glBufferSubData(GL_ARRAY_BUFFER, (sizeof(float) * M.size()*(step-4)), sizeof(float) * M.size(), M.data());
        }
