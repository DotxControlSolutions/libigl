...
namespace glfw
{
  ...
  IGL_INLINE int  Viewer::launch_init(bool resizable,bool fullscreen)
  {
    ...
    if(fullscreen)
    {
      ...
      window = glfwCreateWindow(mode->width,mode->height,"DotX 5-Axis Slicer",monitor,nullptr);
    }
    else
    {
      if (core.viewport.tail<2>().any()) {
        window = glfwCreateWindow(core.viewport(2),core.viewport(3),"DotX 5-Axis Slicer",nullptr,nullptr);
      } else {
        window = glfwCreateWindow(1280,800,"DotX 5-Axis Slicer",nullptr,nullptr);
      }
    }
    if (!window)
    {
